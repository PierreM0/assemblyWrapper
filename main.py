    ...
    def generate(self, generator, out_file):
        if self.token.type == TokenType.ADD:
            ...
            if identifier not in generator.variables and identifier not in generator.functions:
                print(f"{self.token.location}: ERROR: identifier `{identifier}` is not declared", file=sys.stderr)
                sys.exit(1)
            else:
...
class IfNode(AST):
    def __init__(self, token: Token, condition: AST, body: List[AST | None]):
        super().__init__(token)
...
class AssignNode(AST):
    def __init__(self, token: Token, variable: Token, expression: AST):
        super().__init__(token)
    ...
    def generate(self, generator, out_file):
        ...
        identifier = self.variable.literal
...
class Parser:
    ...
    def parse_Q(self):
        if self.current_token() == TokenType.OPEN_PAREN:
            ...
            return BinaryOperator(token, None, None) #  TODO
        elif self.current_token() == TokenType.INT:
            return BinaryOperator(self.chop_token(), None, None) # TODO
            ...
            if self.cursor + 1 < len(self.tokens) and self.tokens[self.cursor + 1].type == TokenType.OPEN_BRACKET:
                ...
                right = self.parse_expr()
                self.chop_token()  # token ]
                return BinaryOperator(token, None, right)  # TODO
            else:
                return BinaryOperator(self.chop_token(), None, None) # TODO
            ...
            while self.current_token() != TokenType.CLOSE_BRACKET:
                ...
                match self.current_token():
                    case TokenType.COMMA | TokenType.CLOSE_PAREN:
                        arg = self.chop_token()
                    case _:
                        print(f"{comma.location}: ERROR: no comma after value in array")
                        exit(1)
            self.chop_token()
            return BinaryOperator(token, None, None) # TODO
    ...
    def current_token_location(self):
        if self.cursor >= len(self.tokens):
            raise Exception
